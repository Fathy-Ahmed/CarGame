...
GLfloat light1_position[] = { 0.0, 5.0, -5.0, 1.0 };
GLfloat light2_direction[] = { 0.0, -1.0, 0.0 };
GLfloat light3_position[] = { 0.0, 3.0, -3.0, 1.0 };
...
void check(unsigned char* data) {
    if (data)
    {
        glGenTextures(1, &texture); // set id for texture
...
void first_page() {
    ...
    glEnable(GL_DEPTH_TEST); // for Collision
    glEnable(GL_TEXTURE_2D);
    ...
    glutSwapBuffers(); // show the new image after show text 
}
...
void Game_Over()
{
    ...
    //-------------------game over image----------------------//
    load(2);  // for show the image of game over 
    glBegin(GL_QUADS);
    glColor3f(1, 1, 1);
...
void keyboard(unsigned char key, int x, int y) {
    ...
    if (key == ' ')   // click space to start game
    {
        play = 1;     
    }
    // lighting 
    if (key == 'r' || key == 'R') { // red lighting
        GLfloat light0_ambient[] = { 0.9, 0.2, 0.2, 1.0 }; 
        GLfloat light0_diffuse[] = { 1.0, 0.3, 0.3, 1.0 };
    ...
    if (key == 'o' || key == 'O') { // stop all lighting
        glDisable(GL_LIGHT0);
        glDisable(GL_LIGHT1);
        glDisable(GL_LIGHT2);
        glDisable(GL_LIGHT3);
